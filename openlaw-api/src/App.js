...
class App extends Component {
  ...
  processNDA = async(event) => {
    ...
    try {
      ...
      const ndaTemplate = await apiClient.getTemplate(openLawConfig.templateName);
      console.log(ndaTemplate);

      const partyAOLUser = await apiClient.getUserDetails(this.state.partyAEmail);
      console.log("partyAOLUser", partyAOLUser);
      const partyBOLUser = await apiClient.getUserDetails(this.state.partyBEmail);
  ...
  buildOpenLawParamsObj = function(ndaTemplate, userA, userB) {
    const object = {
      ...
      title: openLawConfig.templateName,
  ...
  navigateToNda = async() => {
    window.location = openLawConfig.server + "/contract/" + this.state.ndaAgreementId;
  }
        ...
        <Segment attached>
          ...
          <Step.Group ordered>
            ...
            <Step active={this.state.step2Active}>
              <Step.Content>
                <Step.Title>Non Disclosure (NDA)</Step.Title>
                <Step.Description>Agreement on OpenLaw</Step.Description>
                </Step.Content>
            </Step>
            <Step>
              <Step.Content>
